...
while rodando:
    ...
    if not pausa:
        ...
        venceu(pontuacao,0)  # 10 é o valor para vencer
        perdeu(pontuacao, -100)  # -50 é o valor para perder
        ...
        for navio in navio_group:
            ...
            for destino_x, destino_y in destino:
                if (navio.rect.centerx, navio.rect.centery) == (destino_x, destino_y):
                    Barra_Espera(navio,pontuacao, pausa)
                    pontuacao = Barra_Espera(navio, pontuacao,pausa)



            for berco in bercos_group:
                if navio.rect.colliderect(berco.rect):
                    Barra_Descarga(navio,pontuacao, pausa)
                    pontuacao = Barra_Descarga(navio, pontuacao,pausa)
